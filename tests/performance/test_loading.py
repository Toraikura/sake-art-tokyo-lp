...
def preview_request(url):
    return '/aroma-lab/thumbs/' in url or '/assets/game-preview.webp' in url or '/assets/shubo-dive-preview.webp' in url
...
with sync_playwright() as p:
    ...
    for path in ('', 'en/'):
        ...
        page.evaluate("""async () => {
          const selectors = '.fpg-preview--clash,.fpg-labo-scent,.fpg-match-row small:last-child,.fpg-preview--shubo';
          const urls = new Set([...document.querySelectorAll(selectors)].map(element =>
            getComputedStyle(element).backgroundImage.match(/url\\([\"']?([^\"')]+)[\"']?\\)/)?.[1]
          ));
          if (urls.size !== 4 || urls.has(undefined)) throw new Error('Missing preview artwork');
          await Promise.all([...urls].map(async url => {
            const img = new Image(); img.src = url; await img.decode();
            if (!img.naturalWidth) throw new Error('Empty preview artwork');
          }));
        }""")
        assert len({url for url in requests if preview_request(url)}) == 4, requests
    ...
    for path in ('', 'en/'):
        ...
        context.close()
    browser.close()
print('PASS: JA/EN age gate has no audio/arcade requests; at most two sounds warm after entry; four real previews load near the arcade; no eager game iframe or overflow.')
